...
GameMenu::GameMenu( MenuAction* pAction ,HGE * pEngine, hgeFont* pFont ) :
mpMenuAction(pAction),
mpEngine(pEngine),
mpFnt(pFont)
{
...
bool GameMenu::UpdateInput(void)
{
	if (!mpEngine || !mpGui)
	{
		return true;
	...
	if(id) 
	{
		...
		case 2:
			...
			mpMenuAction->OnLevel(mLevelIndex);
